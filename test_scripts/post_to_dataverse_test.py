...
def main():
    # 1️ Data for the new row (logical names only)
    row_data = {
    ...
    file_path = (
        Path("test_scripts") / "graph_test_output" / "graph_users_20260208_183612.csv"
    )
    print("trying to upload file -> " + str(file_path))

    # 3️ Upload the CSV file to the File column
    dataverse_access_token = auth.get_dataverse_token()
    ...
    result = upload_file_to_dataverse(
        access_token=dataverse_access_token,
        environment_url=config.ENV_URL,
    ...
    download_csv_file(
        entity_uri=entity_uri, file_column=file_column, output_path=output_path
    )
